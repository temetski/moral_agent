...
class FineTuneArgs(Args):
    ...
    load_model_ref: str = "runs/Driving__ppo__1__1724832763/ppo.cleanrl_model"
    load_from: int = 0
...
if __name__ == "__main__":
    ...
    for iteration in range(args.load_from+1, args.load_from + args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            # key = tuple(next_obs.flatten()[1:6:2])
            key = tuple([next_obs.flatten()[1],next_obs.flatten()[3],next_obs.flatten()[5]])
            if key in hpolicy:
                hprobs = hpolicy[key]
                actions[step] = action
