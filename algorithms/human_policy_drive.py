...
env = gym.make("environments.drive:Driving", render_mode='ansi', validate=True, ishuman_p=True)
...
for cnt in range(1000):
    ...
    while True:
        ...
        for action in actions:
            try: 
                probs.append(np.e**(Q[(state, action)]/0.7)) #the temperature parameter for Q learning policy (default: 0.7)'
            except:
        ...
        if prev_pair is not None:
            current_key = (to_hashable(state), action)
